...
public class HttpConnectionConfig {
  ...
  /**
   * 套接字读数据超时时间，即从服务器获取响应数据的超时时间，单位毫秒 Socket read data timeout, that is, the timeout for obtaining
   * response data from the server, in milliseconds
   */
  @Value("${socket-timeout:30000}")
  private int socketTimeout = 30_000;

  /**
   * 与服务器连接超时时间，http客户端会创建一个异步线程用以创建套接字连接，此处设置该套接字的连接超时时间，单位毫秒 The connection timeout time to the
   * server, the http client will create an asynchronous thread to create a socket connection, and
   * the connection timeout time of the socket is set here, in milliseconds
   */
  @Value("${connect-timeout:10000}")
  private int connectTimeout = 15_000;

  /**
   * 从连接池获取Connection超时时间，单位毫秒 Get connection timeout from connect Manager, in milliseconds
   */
  @Value("${connection-request-timeout:60000}")
  private int connectionRequestTimeout = 60_000;

  /**
   * 请求重试次数 Number of request retries
   */
  @Value("${retry-times:3}")
  ...
  public static class Pool {
    ...
    /**
     * 监控线程池的线程数量 Monitor the number of threads in the thread pool
     */
    @Value("${monitor-thread-num:1}")
    private int monitorThreadNum = 1;
    /**
     * 把空闲时间超过minEvictableIdleTimeMillis毫秒的连接断开，代码中会向前滚动2s保证在超时之前可以关闭空闲连接
     * 很多服务端默认超时为60s，所以这里默认超时时间为60s Disconnect connections that have been idle for more than
     * minEvictableIdleTimeMillis milliseconds The code will scroll forward 2s to ensure that idle
     * connections can be closed before the timeout. Many servers have a default timeout of 60s, so
     * the default timeout here is 60s.
     */
    @Value("${evictable-idle-time-millis:60000}")
    private int evictableIdleTimeMillis = 60000;
    /**
     * 毫秒检查一次连接池中空闲的连接 需小于evictable-idle-time-millis / 2, 否则会导致连接关闭不及时
     * 默认为-1时会取值evictable-idle-time-millis / 2 - 500 Check for idle connections in the connection
     * pool once in milliseconds It needs to be less than evictable-idle-time-millis/2, otherwise
     * the connection will not be closed in time When the default is -1, it will take the value
     * evictable-idle-time-millis/2 - 1000*
     */
