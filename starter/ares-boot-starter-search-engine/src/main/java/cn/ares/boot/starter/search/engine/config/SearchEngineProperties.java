...
public class SearchEngineProperties {
   ...
   * Cluster name
   */
  @Value("${ares.search-engine.cluster-name:}")
  private String clusterName;
  /**
   * 集群节点（这里可以是多个也可以是一个）
   * Cluster node
   */
  @Value("${ares.search-engine.cluster-nodes}")
  private List<String> clusterNodes;
  /**
   * 请求超时时间
   * Request timeout
   */
  @Value("${ares.search-engine.request-timeout:30000}")
  private Integer requestTimeout;
  /**
   * 连接超时时间
   * Connect timeout
   */
  @Value("${ares.search-engine.connect-timeout:30000}")
  private Integer connectTimeout;
  /**
   * 套接字超时时间
   * Socket timeout
   */
  @Value("${ares.search-engine.socket-timeout:30000}")
  private Integer socketTimeout;
  /**
   * 链接保持时间
   * Keep alive
   */
  @Value("${ares.search-engine.keep-alive:60000}")
  private Long keepAlive;
  /**
   * 默认查询分页大小（不分页情况下覆盖默认的10条）
   * Default query page size
   */
  @Value("${ares.search-engine.default-page-size:10}")
  private Integer defaultPageSize;
  /**
   * 用户名
   */
  @Value("${ares.search-engine.username:elastic}")
  private String username;
  /**
   * 密码
   */
  @Value("${ares.search-engine.password}")
  private String password;
  /**
   * exception: entity content is too long [*] for the configured buffer limit [*]
   * 104857600 = 100 * 1024 * 1024
   */
  @Value("${ares.search-engine.buffer-limit:104857600}")
