...
public class LoggerUtil extends BaseLoggerUtil {
  ...
  /**
   * @author: Ares
   * @description: 打印Deferred错误日志
   * @time: 2024-08-09 19:11:41
   * @params: [msg] 信息
   */
   ...
   * @description: 打印Deferred错误日志
   * @time: 2024-08-09 19:11:41
   * @params: [msg, throwable] 信息，可抛出
   */
  public static void errorDeferred(String msg, Throwable throwable) {
    DeferredLog deferredLog = getDeferredLog();
    if (deferredLog.isErrorEnabled()) {
      deferredLog.error(msg, throwable);
    }
  }

  /**
   * @author: Ares
   * @description: 打印Deferred告警日志
   * @time: 2024-08-09 19:11:41
   * @params: [msg] 信息
   */
  ...
  public static DeferredLog getDeferredLog() {
    ...
    /*
     在SpringBoot加载的过程中EnvironmentPostProcessor的执行比较早，这个时候日志系统根本就还没有初始化，
     所以在此之前的日志操作都不会有效果，使用DeferredLog缓存日志，并在合适的时机回放日志
     EnvironmentPostProcessor was executed early during SpringBoot loading, At this point the logging system has not been initialized at all,
     So no logging operations before that will have any effect, use DeferredLog Indicates deferred log, And play back the log at the right time
     */
